...
def create_norm_logger():
    ...
    def norm_logger_hook(name):
        def hook(module, input, output):
            input_norm = math.log10(input[0].norm().item())
            output_norm = math.log10(output.norm().item())
...
def register_norm_hooks(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, (nn.Conv2d, nn.BatchNorm2d, nn.ConvTranspose2d,
                               nn.MaxPool2d, nn.ReLU)):
